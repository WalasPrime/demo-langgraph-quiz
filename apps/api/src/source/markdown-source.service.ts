...
export class MarkdownSourceService {
			...
			try {
				...
				if (response.status >= 300 && response.status < 400) {
					...
					if (!location || redirectCount === MAX_REDIRECTS)
						throw new BadRequestException('Markdown source has too many redirects or an invalid redirect');
				...
				if (contentType && !['text/markdown', 'text/plain', 'application/octet-stream'].includes(contentType))
					throw new BadRequestException('Markdown source must have a text content type');
				...
				if ((error as Error).name === 'AbortError')
					throw new RequestTimeoutException('Markdown source request timed out');
	...
	private async validateUrl(value: string): Promise<string> {
		...
		if (addresses.some(({ address }) => isPrivateAddress(address)))
			throw new BadRequestException('Markdown source resolves to a private or link-local address');
