...
describe('MongoQuizSessionStore', () => {
  ...
  it('persists and idempotently replays an answer across store instances', async () => {
    ...
    const first = new MongoQuizSessionStore({ getOrThrow: jest.fn((key: string) => key === 'MONGODB_URI' ? 'mongodb://localhost:27017/toploox' : 'toploox') } as unknown as ConfigService<any, true>);
    const second = new MongoQuizSessionStore({ getOrThrow: jest.fn((key: string) => key === 'MONGODB_URI' ? 'mongodb://localhost:27017/toploox' : 'toploox') } as unknown as ConfigService<any, true>);
    (first as any).collectionPromise = Promise.resolve(collection);
    (second as any).collectionPromise = Promise.resolve({ ...collection, findOne: jest.fn().mockResolvedValue({ ...document, answers: [{ questionId: 'q1', selectedOptionIds: ['a'] }], version: 1 }) });
